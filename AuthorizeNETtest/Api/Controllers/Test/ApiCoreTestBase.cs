...
#pragma warning disable 649
	...
	public abstract class ApiCoreTestBase
	{
		...
		protected static Environment TestEnvironment = Environment.SANDBOX;
		...
		static readonly string _md5HashKey;
		...
		private readonly AnetRandom _random = new();
		static ApiCoreTestBase()
		{
			...
			ApiLoginIdKey = UnitTestData.GetPropertyFromNames(Util.Constants.EnvApiLoginid, Util.Constants.PropApiLoginid);
			TransactionKey = UnitTestData.GetPropertyFromNames(Util.Constants.EnvTransactionKey, Util.Constants.PropTransactionKey);
			_md5HashKey = UnitTestData.GetPropertyFromNames(Util.Constants.EnvMd5Hashkey, Util.Constants.PropMd5Hashkey);
		...
		private decimal SetValidAmount(int number, int maxAmount)
		{
			...
			var dollarsAndCents = (decimal)number / 100;

			//Test that result is not less than the global Min Value
			return (dollarsAndCents < MinAmount) ? (MinAmount + dollarsAndCents) : dollarsAndCents;
		}
		...
		protected static TS ExecuteTestRequestWithSuccess<TQ, TS, TT>(TQ request, Environment execEnvironment = null)
		...
		protected static TS ExecuteTestRequestWithFailure<TQ, TS, TT>(TQ request, Environment execEnvironment = null)
			where TQ : ANetApiRequest
			where TS : ANetApiResponse
			where TT : ApiOperationBase<TQ, TS>
		{
		...
		private static TS ExecuteTestRequest<TQ, TS, TT>(bool successExpected, TQ request, Environment execEnvironment = null)
			where TQ : ANetApiRequest
			where TS : ANetApiResponse
			where TT : ApiOperationBase<TQ, TS>
		{
		...
		{
			var mockEnvironment = Environment.CUSTOM;
				...
				foreach (var pd in fieldInfos)
				{
						...
						!(bean.ToString().Equals(name)))
					{
						try
						{
							...
							if (bean is INotifyPropertyChanged propertyChanged)
							{
								var changed = false;
